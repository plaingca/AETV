...
def select(model, cache, device) -> dict:
    model.eval()
    summary = summarise(score_cache(cache, {"refined": model}, device))
    model.train()
    return summary
...
def main() -> None:
    ...
    config = {"gop": GOP, "width": args.width, "blocks": args.blocks, "causal": args.causal}
    ...
    base = select(model, cache, device)["refined"]
    print(f"step 0: select mpp12 {base['mpp12']['mean']:.3f} boundary {base['boundary_tpsnr']['mean']:.3f}", flush=True)
    base_records = score_cache(cache, {}, device)["current"]
    ...
    for step in range(1, args.steps + 1):
        ...
        loss = refiner_loss(recon.float(), video, args.temporal_weight)
        ...
        if step % args.eval_interval == 0 or step == args.kill_step:
            records = score_cache(cache, {"refined": model.eval()}, device)
            model.train()
            ...
            if step == args.kill_step:
                ok = d_mpp["mean"] >= 0 and d_bnd["mean"] > 2 * d_bnd["se"]
                verdict = {"step": step, "passed": ok, "mpp12": d_mpp, "boundary_tpsnr": d_bnd}
