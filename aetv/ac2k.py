"""AC2K: Fixed-rate asymmetric-context video codec for V8 resolution and RF budget.

Engineering adaptation of AC16 architecture for Standard SSB channel (Band W):
- Resolution: 192x108 (aspect ratio 16:9, matching V8)
- RF Bandwidth budget: Band W standard channel (2,816 real coordinates per GOP)
- Frequent 4-frame GOP structure: 1 I-frame + 3 P-frames
- Per-GOP payload allocation:
    * I-frame: 1,280 coordinates
    * P-frame: 512 coordinates (128 motion + 384 content)
...
ARCHITECTURE = "ac2k-asymmetric-context-v1"

# Wire coordinate allocations:
I_VALUES = 1280
MOTION_VALUES = 128
CONTENT_VALUES = 384
...
PAD_TOP = 2
PAD_BOTTOM = 2
PADDED_HEIGHT = 112  # 108 + 2 + 2 = 112 (112 / 16 = 7 exact integer)
PADDED_WIDTH = 192   # 192 / 16 = 12 exact integer
SPATIAL_SHAPE = (7, 12)
SPATIAL_SIZE = 7 * 12  # 84 locations


def pad_frame(x: torch.Tensor) -> torch.Tensor:
    """Pad 108-height frame to 112 for exact 16x downsampling and upsampling."""
    if x.shape[-2] == HEIGHT:
...
class Refine(nn.Module):
    ...
    Tier 2 Architectural Upgrade: Enhanced with a multi-scale Deep Synthesis Tail
    featuring dilated convolutions, half-scale context, and full-resolution detail
    refinement to reconstruct crisp facial anatomy, eyes, lips, fingers, and edges.
    Fully backwards-compatible with base checkpoints via zero-initialized edge head.
    """

    def __init__(self, features: int, width: int, deep_tail: bool = True):
        super().__init__()
        # Base shallow path (retains exact keys: net.0, net.2, net.3, net.4)
        self.net = nn.Sequential(
        ...
        if deep_tail:
            tail_ch = max(64, width * 2)  # 96 if width=48
            self.tail_in = nn.Sequential(
                nn.Conv2d(features + 3, tail_ch, 3, padding=1),
                nn.LeakyReLU(0.1),
                Residual(tail_ch),
            )
            # Multi-scale half-resolution context
            ...
            # High-frequency edge head (zero-initialized so initial output is strictly base_rgb)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if not getattr(self, "deep_tail", False):
            return base_logits
        # Multi-scale high-frequency detail synthesis with bounded residual
...
class FixedAnalysis(nn.Module):
    """Learned exact-dimensional projection for 7x12 feature planes."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.shape[-2:] != self.spatial_shape:
            raise ValueError(f"AC2K analysis requires a {self.spatial_shape} feature plane, got {x.shape[-2:]}")
        return self.spatial(self.map(x).flatten(2)).flatten(1)


class FixedSynthesis(nn.Module):
    """Learned exact-dimensional synthesis for 7x12 feature planes."""
...
class IAnchorAdapter(nn.Module):
    ...
    Prevents dramatic style, luminance, and color tone shifts across GOP boundaries
    while preserving the I-frame's ability to decode standalone (when context is None)
    and gracefully attenuating when a scene cut occurs.
    """
    ...
    def __init__(
        ...
        # Learnable loose anchoring scale (sigmoid * anchor_gate_max ceiling guarantees loose coupling)
        self.gate = nn.Parameter(torch.tensor([-2.0]))
        # Style moment matching weight for mean/std blending
        self.style_weight = nn.Parameter(torch.tensor([-2.5]))
        # Zero-initialize the adapter output so it begins as strict identity
    ...
    def forward(
        ...
        """
        features: (B, features, 112, 192) current I-frame synthesis features
        prev_features: (B, features, 112, 192) or (B, features, 108, 192) preceding P context
        prev_rgb: (B, 3, 112, 192) or (B, 3, 108, 192) preceding P RGB
        """
        ...
        # 1. Soft global moment anchor (AdaIN-style loose mean/variance blending)
...
class MotionCodec(nn.Module):
    """Learned pyramid flow estimator projecting to 128 motion wire coordinates."""
...
class Context(nn.Module):
    """Learned offset hypotheses, fused at full, half, and quarter scales."""
    ...
    def __init__(self, features: int = 16, width: int = 32, num_offsets: int = 2):
        ...
        # 2 flow channels per offset (dx, dy) + 1 softmax weight logit per offset = 3 * num_offsets
...
class PAnalysis(nn.Module):
    def __init__(self, width: int = 32, features: int = 16, diagonal_gdn: bool = True):
        super().__init__()
...
class AC2KCodec(nn.Module):
    """AC2K asymmetric context video codec: 192x108 @ 6 fps, 4-frame GOPs, Band W budget (2816)."""
    ...
    def __init__(
        ...
        self.motion = MotionCodec(m_width)
        ...
        # Stateful GOP-to-GOP streaming memory
    ...
    def encode(
        ...
        if position == 0:
            # Loose anchoring: if previous TX state exists (from previous GOP's last P frame), use it
    ...
    def decode(
        ...
        if position == 0:
            # Check if previous context is available and valid for loose anchoring
            prev_context = None
    ...
    def encode_gop(
        ...
        if video.ndim != 5 or video.shape[1:] != (3, GOP_FRAMES, HEIGHT, WIDTH):
            raise ValueError(
                f"AC2K requires video shape (B, 3, {GOP_FRAMES}, {HEIGHT}, {WIDTH}), got {video.shape}"
            )
    ...
    def decode_gop(
        ...
        *,
        ...
        if payload.ndim != 2 or payload.shape[1] != GOP_VALUES:
            raise ValueError(
                f"AC2K requires exactly {GOP_VALUES} wire coordinates, got {payload.shape}"
            )
        if confidence is None:
